...
class NodeTest extends \PHPUnit_Framework_TestCase
{
	 ...
	 * @tests
	 */
	public function getMin_oneDimension()
	{
		$node = new Node();
		$node->loadData(array('y' => 1));

		$this->assertEquals(1, $node->getMinCoordinateValue());
	}

	/**
	 * @tests
	 */
	public function getMax_oneDimension()
	{
		$node = new Node();
		$node->loadData(array('y' => 1));

		$this->assertEquals(1, $node->getMaxCoordinateValue());
	}

	/**
	 * @tests
	 */
	public function getMin_multiDimension()
	{
		$node = new Node();
		$node->loadData(array('y' => 2, 'x' => 10));

		$this->assertEquals(2, $node->getMinCoordinateValue());
	}

	/**
	 * @tests
	 */
	public function getMax_multiDimension()
	{
		$node = new Node();
		$node->loadData(array('y' => 2, 'x' => 10));

		$this->assertEquals(10, $node->getMaxCoordinateValue());
	}

	/**
	 * @tests
	 */
	public function getMin_empty()
	{
		$node = new Node();
		$this->assertEquals(0, $node->getMinCoordinateValue());
	}

	/**
	 * @tests
	 */
	public function getMax_empty()
	{
		$node = new Node();
		$this->assertEquals(INF, $node->getMaxCoordinateValue());
	}
}
